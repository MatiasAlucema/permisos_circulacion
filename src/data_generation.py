...
def generate_synthetic_data(num_records=NUM_RECORDS, seed=RANDOM_SEED):
    ...
    montos = np.array(
        [max(1000, np.random.normal(monto_base[t], monto_base[t] * 0.3)) for t in tipos]
    )
    ...
    infracciones = np.random.choice(
        [0, 1, 2, 3, 4, 5], num_records, p=[0.50, 0.25, 0.12, 0.07, 0.04, 0.02]
    )
    ...
    probs = df.apply(lambda row: _calcular_probabilidad_inactivo(row, monto_base), axis=1)
    df["estado"] = np.where(np.random.random(len(df)) < probs, "Inactivo", "Activo")
